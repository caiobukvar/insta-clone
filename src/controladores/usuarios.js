    ...
    try {
        ...
        if (email !== req.usuario.email) {
            ...
            if (emailUsuarioExiste) {
                return res.status(404).json('Este email já existe.');
            }
        }

        if (username !== req.usuario.username) {
            const usernameUsuarioExiste = await knex('usuais').where({ username }).first();

            if (usernameUsuarioExiste) {
                return res.status(404).json('Este username já existe.');
            }
        }
        ...
        if (!usuarioAtualizado) {
            return res.status(404).json("O usuário não foi atualizado.");
        }
